...
export const FCanvas = () => {
  ...
  // Monotonically-increasing counter — incremented each time a new canvas restore starts.
  ...
  // Cutout hook — proposals + apply for interactive panel; legacy processImage kept for vault uploads
  ...
  useEffect(() => {
    ...
    // Capture the current generation — if a newer restore starts while this is still running
        ...
        if (response.ok) {
          ...
          if (status.status === 'completed') {
            ...
            if (status.imageData && canvas) {
              ...
              if (isForeground) {
                // ── Foreground: open interactive CutoutPanel ──
                // Entire branch is wrapped in try/catch to prevent poll-retry loops
                try {
                  console.log('[CutoutPanel] Foreground generation complete — fetching SAM proposals');
                  setGenerationJobs(prev => prev.map(job =>
                    job.id === jobId
                  ...
                  // Always mark job as completed — never leave at 95%
                  setGenerationJobs(prev => prev.map(job =>
                    job.id === jobId
              ...
              // ── Background: place directly on canvas ──
              const img = new Image();
              img.onload = async () => {
          ...
          // Job no longer exists (server restarted) — stop polling
          console.error('[poll] Job not found (404) — server likely restarted, stopping poll for:', jobId);
          setGenerationJobs(prev => prev.map(job =>
            job.id === jobId
              ? { ...job, status: 'failed' as const, error: 'Job lost — server restarted' }
              : job
          ));
          return;
        } else {
    ...
    if (selection.activeAction === 'edit' && selection.hasObjectsSelected && canvas) {
      ...
      if (activeObj) {
        ...
        tmpImg.onload = () => {
          ...
          // These match what the diffusion pipeline will receive — avoids
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          if (res.status === 404) {
            setGenerationJobs(prev => prev.map(j =>
              j.id === clientJobId ? { ...j, status: 'failed' as const, error: 'Job lost — server restarted' } : j,
            ));
            editObjectRefs.current.delete(clientJobId);
            return;
          }
        ...
        if (status.status === 'completed' && status.imageData) {
          // ── Auto-apply to the original object ──
          ...
          console.log('[edit] Job completed — applying result. dataLen=%d hasRef=%s isImage=%s',
          ...
          if (saved?.ref && saved.ref instanceof fabric.FabricImage && canvas) {
            // Replace the object entirely — setElement alone doesn't reliably
            ...
            fabric.FabricImage.fromURL(imgSrc).then((newImg) => {
              ...
              canvas.requestRenderAll();
              console.log('[edit] ✓ Replaced canvas object with edited result (%dx%d)', newImg.width, newImg.height);
            }).catch((err) => {
              console.error('[edit] ✗ Failed to create FabricImage from result:', err);
            ...
            // No existing FabricImage ref — add as a brand-new layer
            ...
            console.log('[edit] ✓ Added edited result as new layer');
          }
          editObjectRefs.current.delete(clientJobId);
          return;
        }
  ...
  const handleEditGenerate = useCallback((params: EditGenerateParams) => {
    ...
    setGenerationJobs(prev => [...prev, {
      id: clientJobId,
      prompt: `✏️ ${params.prompt}`,
      status: 'pending' as const,
      progress: 0,
    }]);
    setShowGenerationQueue(true);

    // Close the edit panel — user is free to continue working
    setPendingEdit(null);
  ...
  const handleCutoutConfirm = useCallback(async (
    ...
    let cropOffsetX = 0;
    let cropOffsetY = 0;
    let preScaleW   = 0;   // uncropped image width in px — used to compute canvas scale
    let preScaleH   = 0;
    ...
    img.onload = async () => {
      ...
      const newAssetLayer = await currentLayer.addAssetLayer(
        assetId, layerName, finalImageData, img.width, img.height, posX, posY,
      );
      if (!newAssetLayer && canvas) {
        const fabricImg = new fabric.Image(img, { left: posX, top: posY, selectable: true, hasControls: true });
        canvas.add(fabricImg);
        canvas.requestRenderAll();
      }
    };
  ...
  return (
    <WorkspaceLayout>
      ...
      {/* ── AI Cutout Panel (foreground generation only) ────────────────── */}
      {pendingCutout && (
      ...
      {/* ── AI Edit Panel (inpaint / outpaint) ─────────────────────────── */}
      {pendingEdit && (
        <EditPanel
